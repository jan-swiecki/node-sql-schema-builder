module.exports = require('autowire')(function(_, Reindent, EntityValidator, FieldValidator,
  ...
  ORMGenerator.prototype.getConstructor = function() {
    ...
    return Reindent("function "+self.table.name+"(options) {" +
      ...
      "  var promise = this.validate(options);" +
      "  if(promise.isRejected()) {" +
      "    throw new Error('Invalid values for this model: '+promise.reason()); " +
      "  }" +
      "}" +
      "options = options || {};" +
  ...
  ORMGenerator.prototype.getValidator = function() {
    ...
    var code = [
      ...
      "var validatorProperties = table.getValidationProperties();",
      "return entityValidator(validatorProperties, this, '"+self.table.name+"');",
      "}"
    ];
