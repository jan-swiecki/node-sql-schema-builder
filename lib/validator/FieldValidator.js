module.exports = require("autowire")(function FieldValidatorFactory(_, Promise, escodegen, esprima) {
  ...
  function FieldValidator(fieldProperties, value, optionalName) {
    ...
    var idRegEx = /^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89aAbB][a-f0-9]{3}-[a-f0-9]{12}$/i;

    if(_.isUndefined(value)) {
      // use null instead
      return promiseReject("value cannot be undefined");
    }
    ...
    if(_.isNull(value)) {
      if(fieldProperties.isNull) {
        ...
        return promiseReject("value cannot be null");
      }
    }
    ...
    if(fieldProperties.type === "uuid") {
      ...
      return new Promise(function(res, rej){
        if(_.isNumber(value)){
          throw new Error("Not implemented: check number length");
        } else {
      ...
      return rejectIfNotString().then(function(){
        return finish(
          ...
          "value is not in the TIMESTAMP WITH TIME ZONE format"
        );
      });
    ...
    function rejectIfNotString() {
      if(! _.isString(value)) {
        return Promise.reject(error("value must be a string"));
      } else {
        return Promise.resolve();
      }
    }
